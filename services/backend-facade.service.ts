...
export class BackendFacade {
    ...
    public async getHomeData(): Promise<{posts: PostDetail[], services: IArea[], home: HomeData, layoutData: any, membersTeam: TeamMember[]}> {
        ...
        const homePromise = this.homeService.getSingle({
                populate: [
                    {value: '*'},
                    {value: 'datiStatistici,staff,partnership,servizi,datiSeo,imgAboveTheFold,imgAree,imgDati,imgPartnership', level: 0},
                    {value: 'datiStatistici.dati,partnership.link,datiSeo.keyWords', level: 1}
                ]
            });
